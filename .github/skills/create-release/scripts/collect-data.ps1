...
.SYNOPSIS
    ...
    filtered to PRs authored by a specific GitHub user.

.DESCRIPTION
    Identifies merge/squash commits reachable from EndCommit but not StartCommit, extracts PR numbers,
    queries GitHub for metadata plus (optionally) Copilot review/comment summaries, then emits a JSON
    artifact and a CSV sorted by PR number. Only PRs whose author matches -Author (or the authenticated
    gh user when -Author is omitted) are included.

.PARAMETER StartCommit
    Exclusive starting commit (SHA, tag, or ref). Commits AFTER this one are considered.

.PARAMETER EndCommit
    Inclusive ending commit (SHA, tag, or ref). If not provided, uses origin/<Branch>.

.PARAMETER Branch
    Branch to use as the inclusive end when EndCommit is not specified. Default: main.

.PARAMETER Repo
    GitHub repository (owner/name).

.PARAMETER Author
    GitHub login to filter PRs by. When omitted, defaults to the authenticated gh user.
...
.EXAMPLE
    pwsh ./collect-data.ps1 -StartCommit v1.0.0 -Repo owner/repo

.EXAMPLE
    pwsh ./collect-data.ps1 -StartCommit 0123abcd -Author octocat -OutputDir ./out
...
# Resolve Author: use provided value or fall back to the authenticated GitHub user
if (-not $Author) {
    $Author = gh api /user --jq '.login' 2>$null
    if (-not $Author) {
        Write-Err "Could not determine GitHub user. Pass -Author or ensure 'gh' is authenticated."
        exit 1
    }
    Write-Info "Filtering PRs by authenticated user: $Author"
} else {
    Write-Info "Filtering PRs by author: $Author"
}
...
foreach ($pr in $prNumbers) {
    ...
    try {
        ...
        $fields = 'number,title,author,url,body,reviews'
        ...
        # Skip PRs not authored by the requested user
        if ($authorLogin -ne $Author) {
            Write-DebugMsg "Skipping PR #$pr (author '$authorLogin' != '$Author')"
            continue
        }
...
# Sort by PR number
$sorted = $prDetails | Sort-Object Id
